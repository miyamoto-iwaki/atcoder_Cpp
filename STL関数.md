...
```
## priority_queue
優先度付きキューというデータ構造。要素の大きい[小さい]もの順に取り出す処理を行うことが可能。
大きいもの順に取り出したいとき、`priority_queue<型> pq;`によって宣言可能。
...
小さいもの順に取り出したいとき、`priority_queue<型, vector<型>, greater<型>> pq;`と宣言すればよい。

## set
`set`は重複のないデータのまとまりを扱うためのデータ型。「Keyだけの`map`」のようなイメージ。
