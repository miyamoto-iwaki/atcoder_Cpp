...
## pair
`pair`は2つの異なる型を1つの変数で持つことができる「組」を表現できる型。演算子による比較や`sort`によってソートできる。
以下、p を`pair`の変数とする。
...
## set
`set`は重複のないデータのまとまりを扱うためのデータ型。「Keyだけの`map`」のようなイメージ。`set<型> s;`によって宣言可能。
...
| プログラム | 説明 |  計算量 |
|:-----------|:------------|:------------|
| s.insert(x)| s に要素 x を追加。※すでに同じ値が存在する場合、追加されない | $` O(\log N) `$ |
| s.erase(x)     | s から x を削除  |  $` O(\log N) `$|
| s.count(x)      | s に x が存在するなら 'true' |  $` O(\log N) `$ |
| s.size()    |  s の要素数の取得   | $` O(1) `$ |
| s.empty()   |  s が空なら`true`を返す   | ---- |
| *begin(s)   |  s の最小値の取得   | $` O(1) `$ |
| *rbegin(s)   |  s の最大値の取得   | $` O(1) `$ |
